<script>
	...
	async function link_to_session() {
		session = await window.retrieve_session()
		going_session = session
		return session
	}
	...
	onMount(async () => {
		//
		let a_session = await link_to_session()
		...
		let sp = "//"
		let url = `${prot}${sp}${location.hostname}/calendar-ws`
		
		let req_obj = {
			'url' : url
		}
		//
		let connect_params = await window.app_make_request(a_session,req_obj)
		if ( connect_params ) {
			let port = connect_params.port
			let ws_url = connect_params.ws_url
			add_ws_endpoint(topic_group,cnst.DEFAULT_WS_CALENDAR_ACCESS,port,api_path,subscriptions,ws_url)			
		}
		//add_ws_endpoint(topic_group,cnst.DEFAULT_WS_CALENDAR_ACCESS,9797,api_path,subscriptions,ws_url)
	})
